...
defineFeature(feature, test => {
    ...
    test('User should see a list of suggestions when they search for a city', ({ given, when, then }) => {
        ...
        then('the user should receive a list of cities (suggestions) that match what they’ve typed', async () => {
            ...
            expect(suggenstionListItems).toHaveLength(2);
        });
    });
    ...
    test('User can select a city from the suggested list', ({ given, and, when, then }) => {
        ...
        let suggenstionListItems
        and('the list of suggested cities is showing', () => {
            suggenstionListItems = within(CitySearchDOM).queryAllByRole('listitem');
            expect(suggenstionListItems).toHaveLength(2);
        });

        when('the user selects a city (e.g., “Berlin, Germany”) from the list', async () => {
            const user= userEvent.setup();
            await user.click(suggenstionListItems[0]);
        });
